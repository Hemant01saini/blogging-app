...
import com.blogapp.exception.AlreadyLikedException;
...
public class PostLikeImpl implements PostLikeService {
    ...
    public PostLikeResponseDto likePost(Long userId, Long postId) {
        ...
        if (postLikeRepository.existsByLikedByIdAndPostId(userId, postId)){
            throw new AlreadyLikedException("Post already liked");
        }
    ...
    @Override
    public Map<Long, Long> getLikesCountMap(List<Long> postIds) {
        return postLikeRepository
                .countLikesByPostIds(postIds)
                .stream()
                .collect(Collectors.toMap(
                        PostLikeCount::getPostId,
                        PostLikeCount::getLikesCount
                ));
    }

    @Override
    public long getTotalLikesReceived(Long userId) {
        return postLikeRepository.countLikesReceived(userId);
    }
